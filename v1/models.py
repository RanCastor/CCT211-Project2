...
class SQLStorage():
    ...
    def get_record(self, rid):
        ...
        contact = Contact(row[1], row[2], row[3], row[0])
        return contact
    ...
    def get_all_records(self):
        ...
        for row in self.data_access:
            contacts.append(
                Contact(row[1], row[2], row[3], row[0]))
        return contacts
    ...
    def save_record(self, record):
        ...
        if record.rid == 0:
            self.data_access.execute("""INSERT INTO contacts(name,email,nickname) VALUES (?,?,?)
            """, (record.name, record.email, record.nickname))
            record.rid = self.data_access.lastrowid
        else:
            self.data_access.execute("""UPDATE contacts SET name = ?, email = ?, nickname = ?
            WHERE contact_id = ?""", (record.name, record.email, record.nickname, record.rid))
        self.conn.commit()
...
class Contact():
    def __init__(self, name="", email="", nickname="", rid=0):
    ...
    def __str__(self):
        return f'Contact#: {self.rid}; Name: {self.name}, Email: {self.email}, Nickname: {self.nickname}'
